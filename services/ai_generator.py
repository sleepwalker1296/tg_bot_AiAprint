...
class AIGenerator:
    ...
    async def _generate_kieai_dtf(
        ...
        # Фон совпадает с цветом футболки — тогда при screen/multiply блендинге
        # он полностью исчезает и принт смотрится нативно на ткани.
        bg_color_instruction = (
            "Фон за пределами иллюстрации — строго чёрный (#000000), "
            "без шахматного паттерна, без серых квадратов, без градиентов. "
            "Чистый чёрный цвет — никакой «прозрачности» в виде клеток."
            if tshirt_color == "black" else
            "Фон за пределами иллюстрации — строго белый (#FFFFFF), "
            "без шахматного паттерна, без серых квадратов, без градиентов. "
            "Чистый белый цвет — никакой «прозрачности» в виде клеток."
        )
        ...
        prompt = (
            ...
            f"ВАЖНО ПРО ФОН: {bg_color_instruction}\n"
